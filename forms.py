...
class LoginForm(FlaskForm):
    username = StringField(
        "Username",
        [validators.length(min=3, max=60), validators.DataRequired()],
        render_kw={"placeholder": "User Name"},
    )
    password = PasswordField(
        "Password",
        [validators.length(min=8), validators.DataRequired()],
        render_kw={"placeholder": "Enter your password"},
    )
    mobile = StringField(
        "Registered Mobile",
        [validators.length(min=10), validators.DataRequired()],
        render_kw={"placeholder": "enter registered mobile"},
    )
...
class RegisterForm(FlaskForm):
    name = StringField(
        "Name",
        [validators.length(min=5, max=60), validators.DataRequired()],
        render_kw={"autofocus": True, "placeholder": "Full Name"},
    )
    username = StringField(
        "Username",
        [validators.length(min=5, max=60), validators.DataRequired()],
        render_kw={"placeholder": "Name you wish to see"},
    )
    password = PasswordField(
        "Password",
        [validators.length(min=8), validators.DataRequired()],
    ...
    confirm_password = PasswordField(
        ...
        render_kw={"placeholder": "enter same password"},
    )
    email = EmailField(
        "Email",
        [validators.length(min=8), validators.DataRequired(), validators.Email()],
...
class PasswordResetForm(FlaskForm):
    ...
    confirm_new_password = PasswordField(
        ...
        [validators.EqualTo("new_password", message="password mismatch")],
        render_kw={"placeholder": "enter same password"},
    )
...
class SlotBookingForm(FlaskForm):
    ...
    court1_comment = TextAreaField(
        label="Comment", render_kw={"cols": 24, "rows": 3, "placeholder": "comment"}
    )

    court2_slots = RadioField(label=COURT_2, choices=BADMINTON_CHOICES)

    court2_comment = TextAreaField(
        label="Comment", render_kw={"cols": 25, "rows": 3, "placeholder": "comment"}
    )

    cricket_slots = RadioField(label=CRICKET, choices=CRICKET_CHOICES)

    cricket_comment = TextAreaField(
        label="Comment", render_kw={"cols": 25, "rows": 3, "placeholder": "comment"}
    )
